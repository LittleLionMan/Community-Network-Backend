...
class ServiceInterest(Base):
    ...
    proposed_meeting_time: Mapped[datetime | None] = mapped_column(UTCDateTime)
    agreed_meeting_location: Mapped[str | None] = mapped_column(String(500))
    agreed_meeting_time: Mapped[datetime | None] = mapped_column(UTCDateTime)
...
class UserEngagement(Base):
    ...
    updated_at: Mapped[datetime] = mapped_column(UTCDateTime, onupdate=func.now())
...
class DailyStats(Base):
    ...
    id: Mapped[int] = mapped_column(Integer, primary_key=True)
    date: Mapped[datetime] = mapped_column(UTCDateTime, unique=True)
