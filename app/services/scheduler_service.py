    ...
    async def daily_cleanup(self):
        ...
        async with AsyncSessionLocal() as db:
            try:
                ...
                user_cutoff_date = datetime.now(timezone.utc) - timedelta(days=30)
                result = await db.execute(
                    select(User).where(
                        User.email_verified == False, User.created_at < user_cutoff_date
                    )
                )
                ...
                if deleted_count > 0:
                    logger.info(
                        f"🗑️ Deleted {deleted_count} unverified accounts older than 30 days"
                    )
    ...
    async def process_event_attendance(self):
        ...
        async with AsyncSessionLocal() as db:
            try:
                ...
                result = await db.execute(
                    select(Event)
                    .where(Event.end_datetime < cutoff_time, Event.is_active == True)
                    .limit(10)
                )
        ...
        try:
            ...
            async with AsyncSessionLocal() as db:
                ...
                if old_messages_count > 0:
                    logger.info(
                        f"🗑️ Cleaned up {old_messages_count} old deleted messages"
                    )

                empty_conversations_count = (
                    await message_service.cleanup_empty_conversations()
                )
                if empty_conversations_count > 0:
                    logger.info(
                        f"🗑️ Cleaned up {empty_conversations_count} empty conversations"
                    )
        ...
        try:
            ...
            async with AsyncSessionLocal() as db:
                ...
                _ = await db.execute(
                    ...
                    .values(
                        moderation_reason=None, moderated_at=None, moderated_by=None
                    )
                )
