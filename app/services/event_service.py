...
class EventService:
    ...
    async def can_join_event(self, event: Event, user: User) -> Tuple[bool, str]:
        ...
        try:
            registration_deadline = event.start_datetime - timedelta(hours=deadline_hours)
            if current_time > registration_deadline:
                return False, f"Registration deadline passed ({deadline_hours}h before event)"
        except Exception:
            pass
    ...
    async def auto_mark_attendance(self, event_id: int) -> Dict[str, Any]:
        ...
        try:
            ...
            if not event:
                return {"success": False, "reason": "Event not found", "participants_updated": 0}

            if not event.end_datetime:
                return {"success": False, "reason": "Event has no end time", "participants_updated": 0}
            ...
            if participants_to_update == 0:
                return {"success": True, "reason": "No participants to update", "participants_updated": 0}
    ...
    async def get_user_event_history(self, user_id: int) -> Dict[str, Any]:
        ...
        try:
            result = await self.db.execute(
                select(
                    func.count().filter(EventParticipation.status == ParticipationStatus.REGISTERED).label('upcoming'),
                    func.count().filter(EventParticipation.status == ParticipationStatus.ATTENDED).label('attended'),
                    func.count().filter(EventParticipation.status == ParticipationStatus.CANCELLED).label('cancelled')
            ...
            return {
                ...
                "engagement_level": self._calculate_engagement_level(upcoming + attended + cancelled)
            }
