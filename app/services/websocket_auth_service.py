...
class WebSocketAuthManager:
    ...
    async def authenticate_connection(
        ...
        try:
            ...
            self.authenticated_connections[websocket] = {
                ...
                'token': token  # Store for refresh attempts
            }
    ...
    async def handle_websocket_message(self, websocket: WebSocket, message_data: dict) -> bool:
    ...
    async def _refresh_connection_token(self, websocket: WebSocket, new_token: str) -> bool:
        try:
        ...
        while True:
            try:
                ...
                for websocket, conn_info in self.authenticated_connections.items():
                    ...
                    if current_time - last_refresh > 1500:  # 25 minutes (token expires in 30)
                        try:
                            await websocket.send_text(json.dumps({
                                'type': 'token_expiring',
                                'expires_in': 300,  # 5 minutes warning
    ...
    def get_user_connections(self, user_id: int) -> Set[WebSocket]:
        return {
            websocket for websocket, info in self.authenticated_connections.items()
