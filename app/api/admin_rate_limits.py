...
async def clear_user_rate_limits(
    ...
    content_type: Annotated[
        str | None, Query(description="Specific content type to clear")
    ] = None,
):
...
async def get_rate_limit_overview(
    ...
    SecurityLogger.log_admin_action(
        request, admin_user_id=current_admin.id, action="view_rate_limit_overview"
    )

    active_users = len(
        [
            user_id
            for user_id, attempts in content_rate_limiter.attempts.items()
