...
async def get_events(
    ...
    exclude_political: bool = Query(False, description="Exclude political/civic events"),
    ...
    if political_only or exclude_political:
        ...
        print(f"DEBUG: political_only = {political_only}, exclude_political = {exclude_political}")

        if political_category_ids:
            if political_only == True:
                query = query.where(Event.category_id.in_(political_category_ids))
                print(f"DEBUG: Applying political_only filter with IDs: {political_category_ids}")
            elif exclude_political == True:
                query = query.where(~Event.category_id.in_(political_category_ids))
                print(f"DEBUG: Applying exclude_political filter with IDs: {political_category_ids}")
        else:
            print(f"DEBUG: Keine politischen Kategorien gefunden!")
            if political_only:
                query = query.where(Event.id == -1)
                print(f"DEBUG: political_only aber keine IDs -> returning empty")
...
async def create_event(
    ...
    result = await db.execute(
        select(Event).where(Event.id == db_event.id).options(
            selectinload(Event.creator),
            selectinload(Event.category),
            selectinload(Event.participations)
        )
    ...
    return EventRead(
        ...
        category=category,
        participant_count=participant_count,
        is_full=(event.max_participants is not None and
                participant_count >= event.max_participants)
    )
