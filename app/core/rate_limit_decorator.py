    ...
    def decorator(func: Callable[..., Awaitable[T]]) -> Callable[..., Awaitable[T]]:
        ...
        async def wrapper(*args: object, **kwargs: object) -> T:
            ...
            rate_check = content_rate_limiter.check_rate_limit(
                current_user.id, content_type, user_tier
            )
            ...
            if hasattr(result, "headers") and rate_check.get("remaining"):
                ...
                result.headers["X-RateLimit-Remaining-Hourly"] = str(
                    remaining.get("hourly", 0)
                )
                result.headers["X-RateLimit-Remaining-Daily"] = str(
                    remaining.get("daily", 0)
                )
                if remaining.get("weekly") is not None:
                    result.headers["X-RateLimit-Remaining-Weekly"] = str(
                        remaining["weekly"]
                    )
...
class ReadRateLimiter:
    ...
    def check_read_limit(
        ...
        attempts[:] = [
            (timestamp, count) for timestamp, count in attempts if timestamp > hour_ago
        ]
        ...
        return {"allowed": True, "remaining": limit - current_count - 1, "limit": limit}
    ...
    def decorator(func: Callable[..., Awaitable[T]]) -> Callable[..., Awaitable[T]]:
        ...
        async def wrapper(*args: object, **kwargs: object) -> T:
            ...
            if request:
                ...
                rate_check = read_rate_limiter.check_read_limit(
                    ip_address, endpoint_type
                )

                if not rate_check["allowed"]:
                    SecurityLogger.log_rate_limit_exceeded(
                        request, f"read_{endpoint_type}", details=rate_check
                    )
